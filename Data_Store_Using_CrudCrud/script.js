async function saveToCrud(event){
    event.preventDefault();
  
    var itemName = event.target.itemName.value;
    var desc = event.target.desc.value;
    var price =event.target.price.value;
    var quantity = event.target.quantity.value;
  
    const obj ={
        itemName,
        desc,
        price,
        quantity
    } 
  
    const res=await axios.post('https://crudcrud.com/api/96990cc404204b8f95fa264872ffac2a/UserData',obj)
    showUserOnScreen(res.data);   
}
window.addEventListener('DOMContentLoaded',async()=>{
    const res=await axios.get(`https://crudcrud.com/api/96990cc404204b8f95fa264872ffac2a/UserData`);
    for(let i=0;i<res.data.length;i++){
        showUserOnScreen(res.data[i]);
    }
   })
  
function showUserOnScreen(obj){
    const parentElement = document.getElementById('listOfItems');
    const childElement = document.createElement('li');
  
    //Add text to li
  childElement.textContent = obj.itemName + "-" + obj.desc +"-"+ obj.price+ "-" + obj.quantity; 
  
    //create buy button
    const btn1 = document.createElement('button');
    const btn2 = document.createElement('button');
    const btn3 = document.createElement('button');
  
  
    btn1.textContent ='buy1'
    btn2.textContent='buy2'
    btn3.textContent ='buy3'
  
    btn1.onclick=async()=>{
       const id =obj._id;
       const updatedValues={
        itemName:obj.itemName,
        desc:obj.desc,
        price:obj.price,
        quantity:obj.quantity-1
       }
       console.log(updatedValues);

        await axios.put(`https://crudcrud.com/api/96990cc404204b8f95fa264872ffac2a/UserData/${id}`,updatedValues);
        obj.quantity=obj.quantity-1;
        childElement.textContent = obj.itemName + "-" + obj.desc +"-"+ obj.price+ "-" + obj.quantity;
        childElement.appendChild(btn1);
        childElement.appendChild(btn2);
        childElement.appendChild(btn3);
     
  
    }
  
    btn2.onclick =async ()=>{
      const id =obj._id;
       const updatedValues={
        itemName:obj.itemName,
        desc:obj.desc,
        price:obj.price,
        quantity:obj.quantity-2
       }
       console.log(updatedValues);

       await axios.put(`https://crudcrud.com/api/96990cc404204b8f95fa264872ffac2a/UserData/${id}`,updatedValues)
       
        obj.quantity=obj.quantity-2;
        childElement.textContent = obj.itemName + "-" + obj.desc +"-"+ obj.price+ "-" + obj.quantity;
        childElement.appendChild(btn1);
        childElement.appendChild(btn2);
        childElement.appendChild(btn3); 

    }
  
    btn3.onclick=async()=>{
      const id =obj._id;
      const updatedValues={
       itemName:obj.itemName,
       desc:obj.desc,
       price:obj.price,
       quantity:obj.quantity-3
      }
      console.log(updatedValues);
      await axios.put(`https://crudcrud.com/api/96990cc404204b8f95fa264872ffac2a/UserData/${id}`,updatedValues)
       obj.quantity=obj.quantity-3;
       childElement.textContent = obj.itemName + "-" + obj.desc +"-"+ obj.price+ "-" + obj.quantity;
       childElement.appendChild(btn1);
       childElement.appendChild(btn2);
       childElement.appendChild(btn3); 
    }
  
    //appned btn to childElement
    childElement.appendChild(btn1);
    childElement.appendChild(btn2);
    childElement.appendChild(btn3);
  
  
   //Append li to ul
  parentElement.appendChild(childElement)
  
  
}
